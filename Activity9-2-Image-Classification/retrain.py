...
def prepare_file_system(summaries_dir, intermediate_store_frequency, intermediate_output_graphs_dir):
...
def create_model_info(architecture):
  ...
  if architecture == 'inception_v3':
    ...
    if len(parts) != 3 and len(parts) != 4:
      print("Couldn't understand architecture name '%s'",
                       architecture)
      return None
...
def retrain(bottleneck_dir='/tmp/bottleneck', 
...
#   tf.logging.set_verbosity(tf.logging.INFO)
  ...
  prepare_file_system(summaries_dir, intermediate_store_frequency, intermediate_output_graphs_dir)
